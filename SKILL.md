---
name: ipine-xiaxia-illustrations
description: 生成 ipine 风格的中文正文配图。用于用户要求为中文文章、帖子、博客、Notion 文档、工作流文档、方法论、流程、结构、状态、隐喻或观点生成"怪诞""虾线橘猫""手绘""正文配图""文章插图""配图建议""shot list""去标题/改图"等任务；默认使用虾线橘猫 IP、纯白手绘、少量红橙蓝批注、简洁清爽但天马行空的视觉风格。
---
...
默认视觉 IP 是"虾线橘猫"：橘白相间猫咪，背部正中有一条形似煮熟虾子的橘色毛带，圆眼、空表情，认真做一件荒诞但成立的事。虾线橘猫必须参与画面的核心动作，不能只是站在旁边当装饰。背上的虾线必须清晰可见。
...
**Prompt 格式（即梦验证通过，严格遵守）**：

用自然流动的中文一句话，不要分块、不要列表、不要"猫咪特征"/"画面要求"标题。格式：

```
简笔画风格，手绘线稿，纯白背景。[猫做什么动作，什么表情]。橘白相间猫咪，背部正中央一条纵向橘色条纹，极细黑色线稿，内部不上色，大量留白，[尺寸]构图。
```

控制在 150 字以内。示例：`简笔画风格，手绘线稿，纯白背景。橘白相间猫咪侧身坐着，表情困惑呆萌。背部正中央一条纵向橘色条纹，极细黑色线稿，内部不上色，大量留白，3比4竖版构图。`
...
每张图只讲一个核心结构，虾线橘猫必须参与核心动作，背上的虾线必须清晰可见。不要 PPT、商业插画、幼稚可爱、复杂架构。
