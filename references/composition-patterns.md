...
> 基于 `ian-xiaohei-illustrations` 的 `composition-patterns.md` 改写。
> 核心变化：把"小黑"替换为"虾线橘猫"，保留结构类型和原创隐喻生成法。
...
画法：左侧输入，中间虾线橘猫或怪机器处理，右侧输出，橙色箭头表达主流向。

### 系统局部

适合：信息来源、过滤器、数据库、渲染器、agent 系统局部。

画法：只画 3-5 个核心模块，虾线橘猫参与其中一个关键动作。
...
画法：一层层盒子，不要正式金字塔；虾线橘猫在旁边搬砖或搭建。

### 地图路线

适合：从想法到上线、用户路径、内容承接路径、学习路线。

画法：一条弯曲路径，少量节点，虾线橘猫牵线或走路。
...
3. 让虾线橘猫承担动作：不是站旁边，而是卡在机器里、拉错线、守门、搬运、修补、称重、扶梯子、记录、把东西塞进某个怪装置。
...
不要默认打开或复刻 `assets/examples/`。这些图片只用于风格校准，例如线条密度、留白、颜色克制、虾线橘猫气质。

除非用户明确说"照这张 / 复刻这个构图 / 用这个案例改"，否则不要直接复用旧构图。

同类主题也要换新隐喻。例如"承接路径"不一定画路线，可以画虾线橘猫把内容尾巴接到门把手；"一鱼多吃"不一定画鱼，可以画虾线橘猫把一个纸团压成几种形状。